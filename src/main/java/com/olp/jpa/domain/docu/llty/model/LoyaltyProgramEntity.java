...
@Table(name = "trl_loyalty_program", uniqueConstraints = @UniqueConstraint(columnNames = { "tenant_id",
		"program_code" }))
@NamedQueries({
		@NamedQuery(name = "LoyaltyProgramEntity.findByProgramCode", query = "SELECT t FROM LoyaltyProgramEntity t WHERE t.programCode = :programCode and t.tenantId = :tenantId ") })
@Cacheable(true)
